      ...
      {editor && (
        <div className="fixed bottom-20 right-4 md:bottom-4 z-50 flex flex-col items-end gap-2">
          <button
            ...
            className="text-[10px] uppercase tracking-widest text-neutral-400 hover:text-neutral-700 transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
          >
            {publishState === "running" ? "publishing…" : publishState === "done" ? "published ✓" : "publish"}
          </button>
          <button
            onClick={() => { lock(); setEditor(false); }}
            className="text-[10px] uppercase tracking-widest text-neutral-300 hover:text-neutral-500 transition-colors cursor-pointer"
          >
            lock
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-end justify-end p-4 pointer-events-none">
          <div className="pointer-events-auto bg-neutral-950 text-neutral-200 rounded-xl w-96 max-h-72 flex flex-col shadow-2xl">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-neutral-800">
              ...
              <button onClick={() => { setShowLog(false); setPublishState("idle"); }} className="text-neutral-500 hover:text-neutral-300 text-xs cursor-pointer">✕</button>
            </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/20" onClick={() => setOpen(false)}>
          ...
          >
            ...
            <button
              type="submit"
              disabled={!input}
              className="bg-neutral-900 text-white text-sm rounded-lg py-2.5 hover:bg-neutral-700 transition-colors disabled:opacity-40 cursor-pointer"
            >
              Unlock
            </button>
          </form>
